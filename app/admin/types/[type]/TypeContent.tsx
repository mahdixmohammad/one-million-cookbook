...
export default async function TypeContent({ type }: { type: string }) {
      ...
      <div className="w-[98%] mx-auto mt-4 bg-white shadow-[0px_0px_10px_0.5px_rgba(0,0,0,0.15)] rounded-lg px-3 py-3">
        ...
        <div className="w-full flex flex-col gap-2">
          <div className="grid-cols-[125px_100px_1fr] xs:grid-cols-[190px_175px_1fr] w-full h-24 bg-gray-100 rounded-xl grid grid-rows-1 items-center px-3 xs:px-6 transition-all duration-150">
            <Image className="" src={Object.entries(typeData)[0][1]["image"]} alt="" width={90} height={90}></Image>
            <h3>{type}</h3>
            <h3>{Object.keys(typeData).length}</h3>
          </div>
        </div>
      </div>
      <div className="w-[98%] mx-auto mt-4 bg-white shadow-[0px_0px_10px_0.5px_rgba(0,0,0,0.15)] rounded-lg px-3 py-3">
        ...
        <div className="w-full flex flex-col gap-2">
          {Object.keys(typeData).map((typeName, i) => (
            <Link href={`/admin/types/${type}/${typeName}`} key={i} className="grid-cols-[125px_1fr_auto] xs:grid-cols-[190px_1fr_auto] w-full h-24 bg-gray-100 rounded-xl grid grid-rows-1 items-center px-3 xs:px-6 hover:bg-gray-200 transition-all duration-150">
              <Image className="" src={typeData[typeName]["image"]} alt="" width={90} height={90}></Image>
              <h3>{typeName}</h3>
        ...
        <div className="mt-5 ml-4 mb-2">
          {Object.keys(typeData).length} results
        </div>
      </div>
    </>
  );
